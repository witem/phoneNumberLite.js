...
const phoneNumberNormalizer = require("./normalizer.js")();
...
var phoneNumber = function (dataBase) {
  ...
  function IsNationalNumber(number, md) {
    return md.nationalPattern.test(number);
  }
  ...
  function ParseNationalNumber(number, md) {
    if (!md.nationalPattern.test(number)) {
      return null;
    }
    // Success.
  ...
  return {
    ...
    parse: ParseNumber,
    normalize: phoneNumberNormalizer.normalize
  };
};
